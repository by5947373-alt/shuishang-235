...
頁尾在**兩種主題下都是深色底**，黃色標籤（`.hero-kicker`、`.whisper`）
則**兩種主題都是亮底**。這些地方不能用 `--ink` / `--ink-2` 這類會跟著主題
翻轉的 token —— 要寫死 `--deepsea` 或淺色，否則其中一個主題會變成同色相疊。

英雄區是漸層，停止色用 `--hero1` / `--hero2` 兩個 token，深淺主題各自定義。
