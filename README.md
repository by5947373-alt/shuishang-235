...
標題（h1／h2／h3）用 `--brush`（LXGW WenKai TC），跟標準字的筆觸呼應。
h4 以下維持 `--head`（Noto Sans TC）—— 手寫體在小字級會糊。

**中文網頁字型的坑，這裡要小心：** Google Fonts 把 CJK 切成上百個
unicode-range 子集，瀏覽器只抓用到的那幾個。聽起來很好，但標題那 286 個字
散落在 21 個子集裡，實測要下載 **1304KB**，只為了標題。

所以標題字型改用 `&text=` 指定實際用到的字，同樣的標題只要 **90KB**。
字集由 `collectHeadingChars()` 在建置時從產生好的頁面收集，全站共用一個網址，
只會下載一次。後台存檔會重新產生頁面，字集也跟著更新。

`{{fonts}}` 這個佔位符要等所有頁面都產生完才填得了（要先知道標題用哪些字），
所以主迴圈的「還有沒填的佔位符」檢查對它放行，填完之後再檢查一次。

內文字型不能這樣做 —— 內文字集等於全部內容，網址會爆掉也失去快取意義。
只把字重砍到 400 與 700（原本要 400/500/700/900，但 500 只有兩處、900 只有一處在用）。

**內文字型仍然是最大宗**：Noto Sans TC 一頁大約 1.5MB。真要再壓，
就是改用系統中文字型（PingFang TC／微軟正黑／Noto Sans CJK），
那會少 1.5MB 但每個平台長得不一樣。這是還沒做的決定。

### 標準字

`assets/logo.png`（600×128，41KB，透明底）是頁首的手寫標準字「剛好的生活」。
原始檔在 `src/logo-master.png`（2617×558），要換尺寸從那份重出。

原稿是 PDF，內容是 25 張帶透明遮罩的點陣圖（不是向量，所以轉 SVG 沒有意義）。
重出的做法：

```bash
sips -Z 4000 -s format png --out full.png 原稿.pdf   # 高解析度渲染，會保住 alpha
# 再依 alpha 裁掉四周空白、縮到需要的寬度
```

`.brand-logo` 用**高度**定尺寸、寬度自動，所以換一張比例不同的圖也不用改 CSS。

### 後台的圖片管理

後台有「圖片」分頁，17 個欄位（首頁 2、分類頁 3、單店頁 12）可以直接上傳。
**上傳就生效，不用按儲存** —— 伺服器收到圖之後會立刻重新產生所有頁面。

| 環境變數 | 說明 |
|---|---|
| `PHOTO_MAX_BYTES` | 單張上限，預設 `4000000`（4MB） |

幾個要知道的：

- **圖片存在 volume（`DATA_DIR/photos/`），不在 git 裡。** 重新部署不會消失，
  但也不會被 GitHub 自動同步備份。volume 掉了圖就沒了。
- repo 的 `assets/photos/` 是唯讀的預設值。同名時 volume 的優先，
  所以後台上傳可以蓋掉內建圖，但後台刪不掉內建圖。
- 只認 JPG／PNG／WebP，而且是**看檔頭**判斷，不是看副檔名或 content-type ——
  那兩個都是使用者說了算，可以拿來塞 HTML 或腳本。
- 存檔時用偵測到的真實副檔名，頁面的 `--src` 也會跟著改寫。
- 網址帶 `?v=<修改時間>`。照片跟 css/js 一樣是 `max-age=3600` 但網址不會變，
  沒有版本號的話換圖後訪客會看到舊的長達一小時。

### 店家卡片的 Q 版插畫

12 張手繪 SVG 在 `lib/art.mjs`。**後台可以直接挑**——編輯據點時
下面有「卡片圖示」，選了就寫進 `content.json` 的 `art` 欄位。

沒指定時走「自動」：先用店名對應（`BY_NAME`），對不到就退回分類通用款。
所以新增店家不挑也不會破圖，只是用通用款。

**也可以上傳自己的樣式**：挑選器最後有「＋新樣式」，上傳 JPG／PNG／WebP，
會存成 `u-1`、`u-2`…（編號自動給），跟內建的並列在同一排、任何店家都能選。
還有店家在用的樣式刪不掉，會告訴你是哪幾家。

| 環境變數 | 說明 |
|---|---|
| `ICON_MAX_BYTES` | 自訂圖示上限，預設 `500000`（500KB）。圖示只有 120px 上下，不需要大檔 |

解析順序（`iconHtml()` → `artFor()`）：
自訂圖示（`u-` 開頭，用 `<img>`）→ `art` 欄位指定的內建圖 → 店名 → 分類通用款。

內建的是行內 SVG，會跟著 CSS 變數換色；自訂的是 `<img>`，顏色固定。
用 `<img>` 而不是內嵌，也表示日後就算開放 SVG 上傳，裡面的腳本也不會執行。

圖示存在 volume 的 `DATA_DIR/icons/`，跟照片一樣不進 git。

**`BY_NAME` 為什麼放程式碼而不是種子檔：** 伺服器讀的是 volume 上的
`data/content.json`，`src/content.json` 只是第一次開機的種子檔，種子檔裡
新加的欄位線上不會有。而且後台一存檔，自動同步會把線上內容推回 git，
種子檔多加的欄位會被蓋掉。（實際踩過：本機 12 張都對，線上全部退回通用款。）

後台寫的 `art` 欄位沒有這個問題 —— 它寫的是 `data/content.json`，
本來就是伺服器讀的那份。
...
參考「牛津こどもの森」重新配過，走奶油＋陶土的暖色系。

| Token | 值 | 用在哪 |
|---|---|---|
| `--cream` / `--ground` | `#FDF7F2` | **頁面底色** |
| `--band` / `--sunk` | `#F9E3D3` | 交替區塊的蜜桃帶 |
| `--paper` | `#FFFFFF` | 卡片 |
| `--peach` | `#F7C0A0` | 積木色塊、照片欄位底 |
| `--orange` | `#E8845A` | 積木、裝飾（**不當文字**）|
| `--mustard` | `#EFBE4E` | 標語條、金色刷痕、北回歸線 |
| `--moss` | `#7C9A5B` | 生長分類 |
| `--foot` | `#6B4A3A` | 頁尾底色 |
| `--ink` | `#5F463A` | 主要文字（暖棕，不是黑）|
...
所以另外有一組 `--orange-ink` / `--sun-ink` / `--taste-ink` / `--culture-ink` /
`--grow-ink` / `--gold-ink`，**文字一律用 `-ink` 那組**。

同理，區塊上的 `--acc` 有對應的 `--acc-ink`。

**底色有兩層，`-ink` 是照比較深的那層（蜜桃帶 `#F9E3D3`）算的。**
只照奶油底調會出事：同一個橘色在奶油上是 4.99，放到蜜桃帶上就掉到 4.28，
剛好卡在 AA 門檻下面。改色時兩層底都要驗。
...
「牛津こどもの森」：奶油底、撕紙浪、積木色塊、圓角。
不要往銳利／高冷的精品風走 —— 那個方向試過，對這個客群不合適。

版面分三層：`--cream` 頁底 → `--band` 蜜桃帶交替區塊 → `--paper` 白卡片。

### 撕紙浪怎麼運作

區塊之間那道波浪邊是 `.wave` 的 `::before`：**用區塊自己的底色**塗滿一個
SVG 遮罩，往上挪 99%，蓋在上一區的下緣。

關鍵是**浪屬於下面那一區**。因為它在 DOM 裡比較晚，才會自然蓋過上一區；
反過來做（在上一區加 `::after`）會被下一區的背景蓋掉。

- 三種節奏 `--wave-a/b/c`，用 `.wave` / `.wave-b` / `.wave-c` 切換，
  透過 `--wv` 變數傳給遮罩，交錯著用才不會每一刀都長一樣。
- 加 `.wave-gold` 會多一道芥末色刷痕（同一條浪再往上挪一點，從底下透出來）。
- 區塊要設 `--sec-bg` 告訴浪自己是什麼顏色；`.sec` 和 `.sec-sand` 都設好了。
- `.wave > .wrap` 有 `z-index:2`。**不要寫成 `.wave > *`** ——
  那會把絕對定位的裝飾（`.deco`）強制改成 `position:relative`。

### 裝飾元件

`.totem`（積木堆）、`.vb`（直式標語條）、`.sp`（歪一點的特別企劃卡）、
`.cta`（大顆膠囊按鈕）、`.arrow`（圓形箭頭）、`.ph`（照片欄位）。

`.ph` 沒放圖時會顯示斜紋蜜桃底，不是破圖 —— 因為
`background-image` 第一層是 `var(--src,none)`，沒設就跳過，露出底下的紋理層。
照片檔名與規格見 `assets/photos/README.md`。
