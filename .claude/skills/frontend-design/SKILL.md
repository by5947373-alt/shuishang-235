...
description: 改「剛好的生活」（嘉義水上鄉）網站的樣式、版面或頁面時載入。內含這個站的設計系統規則、撕紙浪的運作方式、對比色的兩層底色陷阱，以及一支可直接跑的對比／溢出檢查腳本。動到 assets/style.css、src/pages/、src/partials/ 或任何顏色時都適用。
---
...
標題用 `--brush`（LXGW WenKai TC 楷體），h4 以下用 `--head`（Noto Sans TC）。

**加任何中文網頁字型之前先算成本。** Google Fonts 把 CJK 切成上百個
unicode-range 子集，看似只抓用到的，但字一散開就會抓一堆：
標題 286 個字散在 21 個子集裡 = 1304KB。

標題字型用 `&text=` 指定實際字元，同樣內容只要 90KB。字集由
`collectHeadingChars()` 在建置時收集，`{{fonts}}` 佔位符在所有頁面產生完才填
（要先知道用了哪些字），所以主迴圈的佔位符檢查對它放行、填完再驗一次。

內文不能用 `&text=`（字集＝全部內容）。目前 Noto Sans TC 一頁約 1.5MB，
這是還沒解的問題。

**量測陷阱：** 中文是全形字，任何 CJK 字型的寬度都一樣（連不存在的字型也是），
所以不能用寬度判斷字型有沒有套上 —— 要畫到 canvas 比像素。
還有，開發機可能本機裝了同名字型，會蓋過網路字型，讓你以為有載其實沒有。
...
站名就是「剛好的生活」，跟標準字一致。
首頁大標仍是「23.5°／剛剛好的城市」—— 那是標語不是站名，
上面的標準字才是品牌名，這是刻意的。23.5°（北回歸線）是全站的主題，別拿掉。
