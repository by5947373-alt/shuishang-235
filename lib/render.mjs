...
/** 把問診題庫寫成前端可用的 assets/quiz-data.js。 */
export function writeQuizData() {
  const data = read('src', 'quiz.json');
  const out = '/* 由 build.js 從 src/quiz.json 產生 —— 不要直接編輯。 */\n' +
    'window.SHUISHANG_QUIZ = ' + JSON.stringify(JSON.parse(data)) + ';\n';
  writeFileSync(p('assets', 'quiz-data.js'), out, 'utf8');
