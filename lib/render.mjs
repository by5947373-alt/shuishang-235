...
/** 照片還沒放時，把 --src 整個拿掉。
 *  留著的話每次載入都會對不存在的檔案發一個 404 —— 版面沒事（會顯示備用底色），
 *  但 console 一片紅、也多了沒必要的請求。檔案放進 assets/photos/ 後
 *  重新建置就會自動接回去。
 *
 *  另外：路徑一定要用絕對的 /assets/…。custom property 裡的相對 url()
 *  是相對於「使用它的樣式表」解析的，不是相對於文件 —— 寫 assets/photos/x.jpg
 *  會變成 /assets/assets/photos/x.jpg。 */
function dropMissingPhotos(html) {
  return html.replace(/\s*--src:url\((\/assets\/photos\/[^)]+)\);?/g, (m, url) =>
    existsSync(p(url.replace(/^\//, ''))) ? m : '');
}
...
export function renderSite(content, baseUrl) {
  ...
  for (const f of Object.keys(out)) {
    if (f.endsWith('.html')) out[f] = versionAssets(dropMissingPhotos(out[f]), out);
  }
