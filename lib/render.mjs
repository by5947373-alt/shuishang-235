...
function renderVenues(items, cat, iconDir) {
...
function renderBlocks(main, content, opts = {}) {
  return main.replace(/\{\{(venues|crops|news|silhouette)(?::(\w+))?\}\}/g, (_m, kind, arg) => {
    ...
    if (kind === 'venues') {
      ...
      return renderVenues(content.venues[arg], arg, opts.iconDir);
    }
...
export function versionAssetsIn(file) {
  ...
  const after = versionAssets(before.replace(/(assets\/[\w.-]+\.(?:css|js|png|svg|webp))\?v=[a-f0-9]+/g, '$1'), {});
  if (after !== before) writeFileSync(f, after, 'utf8');
  return Buffer.byteLength(after);
}

/** 給 assets 底下的檔案加上內容雜湊當版本號，這樣更新後訪客不會拿到快取裡的舊檔。
 *  圖片也要 —— 換 logo 時檔名不變，沒有版本號的話快取一小時內都是舊的。
 *  photos/ 和 icons/ 不走這裡：它們在 volume 上，由 dropMissingPhotos 和
 *  iconHtml 各自用修改時間加版本號。 */
function versionAssets(html, generated) {
  const cache = new Map();
  return html.replace(/(?<=["'])assets\/([\w.-]+\.(?:css|js|png|svg|webp))(?=["'])/g, (m, name) => {
    if (!cache.has(name)) {
...
/** 找出某個欄位實際存在的圖檔。先找 photoDir（伺服器上是 volume，
 *  後台上傳的圖放那裡），再找 repo 裡的 assets/photos/。
 *  回傳實際檔名（含副檔名）或 null。 */
export function findPhoto(slot, photoDir) {
  for (const dir of [photoDir, p('assets', 'photos')]) {
    if (!dir) continue;
    for (const ext of PHOTO_EXT) {
      if (existsSync(join(dir, slot + ext))) return slot + ext;
    }
  }
  return null;
}
...
/** 圖檔的版本號，取自修改時間。
 *  照片跟 css/js 一樣是 max-age=3600，但網址不會變 —— 後台換一張圖，
 *  訪客會看到舊的長達一小時。加上版本號，換圖等於換網址，立刻生效。 */
function photoVersion(file, photoDir) {
  for (const dir of [photoDir, p('assets', 'photos')]) {
    if (!dir) continue;
    const f = join(dir, file);
    if (existsSync(f)) {
      try { return Math.round(statSync(f).mtimeMs).toString(36); } catch { return ''; }
    }
  }
  return '';
}

/** 照片還沒放時，把 --src 整個拿掉；有放但副檔名不同時，改寫成實際的檔名。
 *
 *  留著不存在的路徑，每次載入都會對它發一個 404 —— 版面沒事
 *  （會顯示備用底色），但 console 一片紅、也多了沒必要的請求。
 *
 *  路徑一定要用絕對的 /assets/…。custom property 裡的相對 url() 是相對於
 *  「使用它的樣式表」解析的，不是相對於文件 —— 寫 assets/photos/x.jpg
 *  會變成 /assets/assets/photos/x.jpg。 */
function dropMissingPhotos(html, photoDir) {
  return html.replace(/\s*--src:url\(\/assets\/photos\/([^)]+)\);?/g, (m, file) => {
    const slot = file.replace(/\.[a-z]+$/i, '');
    const found = findPhoto(slot, photoDir);
    if (!found) return '';
    const v = photoVersion(found, photoDir);
    return m.replace(file, found + (v ? '?v=' + v : ''));
  });
}

/** 使用者自己上傳的圖示。key 以 u- 開頭，才跟內建的 duck/soup/… 分得開。 */
export function findIcon(key, iconDir) {
  if (!iconDir || !/^u-[a-z0-9]+$/.test(key || '')) return null;
  for (const ext of PHOTO_EXT) {
    if (existsSync(join(iconDir, key + ext))) return key + ext;
  }
  return null;
}

/** 卡片圖示的 HTML。自訂的用 <img>（上傳的是點陣圖，而且用 img 載入
 *  就算日後開放 SVG 也不會執行裡面的腳本）；內建的維持行內 SVG，
 *  這樣才能跟著 CSS 變數換色。 */
function iconHtml(v, cat, iconDir) {
  const f = findIcon(v?.art, iconDir);
  if (f) {
    let ver = '';
    try { ver = '?v=' + Math.round(statSync(join(iconDir, f)).mtimeMs).toString(36); } catch { /* 剛好被刪 */ }
    return `<img class="q-art" src="/assets/icons/${escAttr(f)}${ver}" alt="" loading="lazy" decoding="async">`;
  }
  return artFor(v, cat);
}

/** 所有可以放圖的欄位。後台和伺服器共用這份清單，才不會兩邊對不起來。 */
export function photoSlots(content) {
  const out = [
    { slot: 'sp-routes', group: '首頁', label: '特別企劃 #01・散策路線', hint: '16:9，綠色隧道／鐵道／田間小路' },
    { slot: 'sp-map', group: '首頁', label: '特別企劃 #02・生活座標地圖', hint: '16:9，田景／街區遠景' },
  ];
  for (const cat of CAT_ORDER) {
    out.push({
      slot: `hero-${cat}`, group: '分類頁',
      label: `${CAT_LABEL[cat]}頁・整面底圖`,
      hint: '會被打淡到約 18%，挑色調單純的環境照',
    });
  }
  for (const cat of CAT_ORDER) {
    (content.venues?.[cat] || []).forEach((v, i) => {
      out.push({
        slot: venueFile(cat, i).replace(/\.html$/, ''), group: `${CAT_LABEL[cat]}・單店頁`,
        label: v.name, hint: '會被打淡，挑店面或環境照',
      });
    });
  }
  return out;
...
/** 字型連結。
 *
 *  中文網頁字型的坑：Google Fonts 把 CJK 切成上百個 unicode-range 子集，
 *  瀏覽器只下載用到的那幾個 —— 聽起來很好，但標題那 286 個字散落在 21 個
 *  子集裡，實測要抓 1304KB，只為了標題。
 *
 *  改用 &text= 指定實際用到的字，同樣的標題只要 90KB。字集在建置時從
 *  產生好的頁面收集，全站共用同一個網址，所以只會下載一次。
 *
 *  內文字型（Noto Sans TC）不能這樣做 —— 內文字集等於全部內容，
 *  網址會爆掉也失去快取意義，所以維持一般子集，只把字重砍到 400 與 700。 */
function fontLink(headingChars) {
  const fams = [
  ...
  let extra = '';
  if (headingChars) {
    const txt = encodeURIComponent(headingChars);
    extra = `<link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=LXGW+WenKai+TC:wght@700&text=${txt}&display=swap">\n`;
  }
  return `<link rel="stylesheet" href="https://fonts.googleapis.com/css2?${fams.map((f) => 'family=' + f).join('&')}&display=swap">\n` + extra;
}

/** 收集所有 h1/h2/h3 裡的中日韓字元。標題字型只需要這些。 */
function collectHeadingChars(files) {
  const set = new Set();
  for (const [f, html] of Object.entries(files)) {
    if (!f.endsWith('.html')) continue;
    for (const m of html.matchAll(/<h[123][^>]*>([\s\S]*?)<\/h[123]>/g)) {
      for (const ch of m[1].replace(/<[^>]+>/g, '')) {
        if (ch.codePointAt(0) > 0x2e80) set.add(ch);       // 中日韓與全形標點
      }
    }
  }
 ...
 *  改一次資料要改兩個地方，遲早會不同步。 */
function renderVenuePages(content, layout, header, footer, nav, base, iconDir) {
  ...
  for (const cat of CAT_ORDER) {
    ...
    items.forEach((v, i) => {
      ...
      const main = tpl
        ...
        .replace('{{art}}', '          ' + iconHtml(v, cat, iconDir))
        .replace('{{silhouette}}', renderSilhouetteOne(v))
        .replace('{{meta}}', metas)
        .replace('{{gmap}}', gmap)
        .replace('{{siblings}}', sibs);

      const title = `${v.name}｜${CAT_LABEL[cat]}・剛好的生活`;
      ...
      const left = html.match(/\{\{(\w+)\}\}/g)?.filter((x) => x !== '{{fonts}}');
      if (left?.length) throw new Error(`${file} 還有沒填的佔位符：${[...new Set(left)].join(', ')}`);
      out[file] = html;
    });
  }
  return out;
 ...
 *  回傳 {檔名: 內容}。 */
export function renderSite(content, baseUrl, photoDir, iconDir) {
  ...
  for (const f of [...nav.map((n) => n[0]), ...extra]) {
    ...
    const html = layout
      ...
      .replace('{{main}}', renderBlocks(main, content, { iconDir }))
      .replace('{{footer}}', footer)
      .replace('{{scripts}}', scripts);

    // {{fonts}} 要等所有頁面產生完、知道標題用了哪些字才能決定，稍後統一填
    const left = html.match(/\{\{(\w+)\}\}/g)?.filter((x) => x !== '{{fonts}}');
    if (left?.length) throw new Error(`${f} 還有沒填的佔位符：${[...new Set(left)].join(', ')}`);
    out[f] = html;
  }
  const venuePages = renderVenuePages(content, layout, header, footer, nav, base, iconDir);
  Object.assign(out, venuePages);

  // 標題字型要先知道用了哪些字才能決定網址，所以在頁面都產生好之後才補上
  const link = fontLink(collectHeadingChars(out));
  for (const f of Object.keys(out)) {
    if (!f.endsWith('.html')) continue;
    out[f] = out[f].replace('{{fonts}}', link);
    if (out[f].includes('{{')) throw new Error(`${f} 還有沒填的佔位符`);
  }

  out['assets/map-data.js'] = mapData(content);
  for (const f of Object.keys(out)) {
    if (f.endsWith('.html')) out[f] = versionAssets(dropMissingPhotos(out[f], photoDir), out);
  }
...
export function writeSite(content, baseUrl, photoDir, iconDir) {
  const files = renderSite(content, baseUrl, photoDir, iconDir);
