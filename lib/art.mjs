...
const face = (x, y, gap = 6) =>
  `<circle cx="${x - gap}" cy="${y}" r="1.5" fill="var(--deepsea)" stroke="none"/>` +
  `<circle cx="${x + gap}" cy="${y}" r="1.5" fill="var(--deepsea)" stroke="none"/>` +
  `<path d="M${x - 2.4} ${y + 4.2}q2.4 2.2 4.8 0" stroke-width="1.9"/>`;
const blush = (x, y, r = 2.6) =>
  `<circle cx="${x - 11}" cy="${y}" r="${r}" fill="var(--blossom)" stroke="none"/>` +
  `<circle cx="${x + 11}" cy="${y}" r="${r}" fill="var(--blossom)" stroke="none"/>`;
...
export const ART = {
  ...
  duck: S(                                            // 烤鴨
    ...
    face(41, 19, 4.6) + '<path d="M6 56h52" stroke-width="2.6"/>'),
  soup: S(                                            // 仙菇干貝雞：香菇站在碗裡
  ...
  coffee: S(                                          // 品皇咖啡
    ...
    face(28, 38, 5.5) + blush(28, 42, 2.4)),
  // ── 生長 ──
  teacup: S(                                          // 仙草茶：深色飲料杯＋吸管
    ...
    '<circle cx="26" cy="35" r="2.2" fill="var(--mustard)"/>' +
    '<circle cx="38" cy="35" r="2.2" fill="var(--mustard)"/>' +
    '<path d="M28.6 41q3.4 3 6.8 0" stroke="var(--mustard)" stroke-width="2.2"/>'),
  ...
  pineapple: S(                                       // 在地蔬果
    ...
    face(32, 36, 5.5) + blush(32, 40, 2.4)),
...
/** 分類的通用款 —— venue 沒指定 art、或指定了不存在的名字時用。 */
const FALLBACK = { taste: 'ricebowl', culture: 'train', grow: 'pineapple' };
export const artFor = (v, cat) => ART[v?.art] || ART[FALLBACK[cat]] || '';
