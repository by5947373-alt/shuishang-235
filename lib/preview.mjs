...
export function buildPreview(content) {
  ...
  // 用正規式整段換掉，才不會被 brand 內容（文字版→圖檔版）的改動弄壞
  const header = read('src', 'partials', 'header.html')
    .replace('{{nav}}', tabs.join(''))
    .replace(/<a class="brand"[^>]*>([\s\S]*?)<\/a>/,
      (_m, inner) => `<button type="button" class="brand" data-go="pv-index">${inner}</button>`);
  ...
  // 字型連結是在產生頁面時才決定的（標題字集要先知道），所以從產生好的首頁抄
  const fonts = (read('index.html').match(/<link rel="stylesheet" href="https:\/\/fonts[^>]*>/g) || []).join('\n');
